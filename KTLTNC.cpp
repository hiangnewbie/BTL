...
struct ClassInfo
{
    ...
    int studentCount;
    ClassInfo() : id(0), className(""), coVanHocTapId(0), studentCount(0) {}
};
...
public:
    ...
    void assignStudentToClass(SinhVien& student, int selectedLopId)
    {
        ...
        c.studentCount++;
        if (c.coVanHocTapId != 0 && find(c.danhSachGiangVien.begin(), c.danhSachGiangVien.end(), c.coVanHocTapId) == c.danhSachGiangVien.end()) {
            c.danhSachGiangVien.push_back(c.coVanHocTapId);
        }
    ...
    bool isTeacherAlreadyAdvisor(int teacherId, int ignoreClassId = -1) const
    {
        for (const auto& c : classes)
        {
            if (c.id == ignoreClassId)
                continue;
    ...
    bool isTeacherAlreadyAdvisor(int teacherId) const
    {
        for (const auto& c : classes)
        {
            if (c.coVanHocTapId == teacherId)
    ...
    void addClass()
    {
        ...
        while (true) {
            ...
            if (c.coVanHocTapId == 0) break;
            if (!teacherIdExists(c.coVanHocTapId)) {
                cout << "Giang vien khong ton tai. Vui long nhap lai.\n";
            }
            string advisorClass = getAdvisorClassName(c.coVanHocTapId);

            if (!advisorClass.empty())
            {
                cout << "Giang vien nay dang la co van hoc tap cua lop "
                << advisorClass
                << ". Khong the gan them.\n";
            }
            else {
        ...
        int i = 0;
        while (i < c.studentCount)
        {
            int idSV = nhapSo<int>("Nhap ID Sinh vien thu " + to_string(i + 1) + ": ");
            ...
            {
                if (!students[idxSV].getLop().empty() && students[idxSV].getLop() != c.className)
                {
                    SetColor(RED);
                    cout << "Sinh vien da co lop (" << students[idxSV].getLop() << "). Vui long nhap ID Sinh vien khac.\n";
                    SetColor(WHITE);
                    return;
                }
                else
                {
                    students[idxSV].setlopChuNhiem(c.className);
                    students[idxSV].setTeacherId(c.coVanHocTapId);
                    SetColor(GREEN);
                    cout << "Da them sinh vien " << students[idxSV].getFullName() << " vao lop " << c.className << ".\n";
                    SetColor(WHITE);
                    i++;
                }
            }
    ...
    void viewSystemStatistics() const
    {
        ...
        for (const auto& c : classes)
        {
            ...
            for (const auto& t : teachers)
            {
                if (!c.className.empty() && !t.getMon().empty() && c.className.find(t.getMon()) == 0)
                {
                    cout << "   - " << t.getFullName() << " (Mon: " << t.getMon() << ")\n";
                    foundDayLop = true;
                }
            }
            if (!foundDayLop)
    ...
    void adminMenu(QuanLyThi& thi)
    {
        ...
        do
        {
            ...
            cout << "║ [17] Xem Bang xep hang (Thay cho loc de)    ║\n";
            ...
            case 5:
                addClass();
                break; // <-- Fix yêu cầu 1: Nhập thêm lớp xong tự out ra menu Admin, không hiện menu học sinh
            ...
            case 12: exportExamReport(thi); break; // <-- Fix yêu cầu 4: Đổi format xuất CSV
            ...
            case 17: viewRanking(); break; // <-- Fix yêu cầu 5: Thay lọc đề bằng bảng xếp hạng
